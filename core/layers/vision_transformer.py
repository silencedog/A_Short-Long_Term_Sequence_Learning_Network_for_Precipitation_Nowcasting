...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=32, patch_size=16, in_chans=32, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.reshape_layer = nn.Conv2d(3, in_chans, kernel_size=1, stride=1)
    ...
    def forward(self, x_spatial, x_temporal):
        ...
        x_spatial = self.forward_features(x_spatial)
        vit_spatial_output = x_spatial.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        vit_spatial_output = self.reshape_layer(vit_spatial_output)
        # vit_weight = self.head(x)
        if x_temporal is not None:
            x_temporal = self.forward_features(x_temporal)
            vit_temporal_output = x_temporal.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            vit_temporal_output = self.reshape_layer(vit_temporal_output)
        else:
            vit_temporal_output = x_temporal
        return vit_spatial_output, vit_temporal_output
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    ...
    # checkpoint_path = '/home/code/PredRANN-master/pretrain_model/vit_base_p16_224.pth'
